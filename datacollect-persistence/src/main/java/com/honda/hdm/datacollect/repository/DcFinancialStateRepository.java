...
public interface DcFinancialStateRepository extends IBaseStatusableRepository<DcFinancialState, Long> {
    ...
    @Query("select finState from DcFinancialState finState " +
            ...
            "(:statusId is null or status.id = :statusId) group by finState.id, finState.month, finState.year, finState.limitDate, finState.dealer, finState.bstate, finState.dcRecordStatusId, finState.createTimestamp, finState.updateTimestamp order by finState.month asc")
    ...
    @Query("select finState from DcFinancialState finState " +
            ...
            "left join statuses.status status " +
            "where " +
            "statuses.dateAssignment = (select max(dateAssignment) from DcFinancialStateXStatus where financialState.id = finState.id) " +
            "group by finState.id, finState.month, finState.year, finState.limitDate, finState.dealer, finState.bstate, finState.dcRecordStatusId, finState.createTimestamp, finState.updateTimestamp order by finState.month asc")
    public List<DcFinancialState> filterAllFinancialState();
}
