...
CREATE SEQUENCE dcollectdb.dc_service_type_seq START WITH 1;

CREATE OR REPLACE TRIGGER dcollectdb.service_id_increment 
BEFORE INSERT ON dcollectdb.dc_service_type 
FOR EACH ROW
WHEN ( new.id IS NULL )
BEGIN
  :new.id := dcollectdb.dc_service_type_seq.nextval;
END;
...
ALTER TABLE dcollectdb.dc_service_type_x_order_type ADD CONSTRAINT dc_st_ot_pk PRIMARY KEY ( dc_service_type_id,
                                                                                                   dc_order_type_id );
...
GRANT ALL PRIVILEGES ON dcollectdb.dc_service_type_x_order_type to dcollectadm;