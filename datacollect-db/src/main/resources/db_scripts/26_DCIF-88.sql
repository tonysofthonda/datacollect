...
CREATE SEQUENCE dcollectdb.dc_position_seq START WITH 1;

CREATE OR REPLACE TRIGGER dcollectdb.position_id_increment 
BEFORE INSERT ON dcollectdb.dc_position 
FOR EACH ROW
WHEN ( new.id IS NULL )
BEGIN
  :new.id := dcollectdb.dc_position_seq.nextval;
END;
...
ALTER TABLE dcollectdb.dc_contact_x_dealer ADD (id INTEGER, dc_position_id INTEGER);

CREATE SEQUENCE dcollectdb.dc_contact_x_dealer_seq START WITH 1;

CREATE OR REPLACE TRIGGER dcollectdb.dc_con_x_dea_id_inc_update
	BEFORE UPDATE ON dcollectdb.dc_contact_x_dealer 
	FOR EACH ROW
	WHEN ( new.id IS NULL )
BEGIN
...
UPDATE dcollectdb.dc_contact_x_dealer SET dc_position_id = 1;

DROP TRIGGER dcollectdb.dc_con_x_dea_id_inc_update;

CREATE OR REPLACE TRIGGER dcollectdb.dc_con_x_dea_id_increment
...
ALTER TABLE dcollectdb.dc_contact_x_dealer MODIFY ( dc_position_id NOT NULL);
ALTER TABLE dcollectdb.dc_contact_x_dealer ADD CONSTRAINT dc_contact_x_dealer_pk PRIMARY KEY ( id );
ALTER TABLE dcollectdb.dc_contact_x_dealer ADD CONSTRAINT dc_con_x_dea_x_position_fk2 FOREIGN KEY(dc_position_id) REFERENCES dcollectdb.dc_position( id );
...
GRANT ALL PRIVILEGES ON dcollectdb.dc_position_seq to dcollectadm;
