...
CREATE SEQUENCE dcollectdb.dc_system_service_seq START WITH 1;

CREATE OR REPLACE TRIGGER dcollectdb.system_service_id_increment 
BEFORE INSERT ON dcollectdb.dc_system_service 
FOR EACH ROW
WHEN ( new.id IS NULL )
BEGIN
  :new.id := dcollectdb.dc_system_service_seq.nextval;
END;
