...
public class DcFileInProcessor implements Processor {
    ...
    @Autowired
    private IDcModelService dcModelService;
...
//    @Transactional
    ...
    private List<String> getEmailsDealer(String dealerNumber) {
        ...
        try{
        	List<DcContactXDealer> dealerContacts = dcDealerService.getEnabledContacts(dealerNumber);
            dealerContacts.forEach((contactDealer) -> {
                mailList.add(contactDealer.getContact().getEmail());
            });
