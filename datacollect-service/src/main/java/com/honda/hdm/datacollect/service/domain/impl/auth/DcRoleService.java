...
public class DcRoleService extends BaseDomainService<DcRole, Long> implements IDcRoleService {
    ...
    @Override
    public DcRoleDto updateDto(DcRoleDto role, Long id) {
        DcRoleDto currentRole = findOneDto(id);
        if(currentRole == null){
            return null;
        }
        currentRole.setName(role.getName());
        currentRole.setDescription(role.getDescription());
        currentRole.setPositions(role.getPositions().stream().map(this::checkAndSavePosition).collect(Collectors.toList()));
        currentRole.setLdapGroups(role.getLdapGroups().stream().map(this::checkAndSaveLdapGroup).collect(Collectors.toList()));
        currentRole.setPermissions(role.getPermissions());
        return dtoConverter.convertRole(save(modelConverter.convertRole(currentRole)));
    }
