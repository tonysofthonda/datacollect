                    	...
                    	if(operationCode == null){
                            ...
                            try {
                            	...
                            	if(dcOperationCode == null) {
                            		...
                            		Boolean containsBpService = dcOperationCode.getServiceTypes().stream().anyMatch(serviceType -> serviceType.getName().equals(ServiceTypeEnum.BP.toString()));
                                    Boolean containsSeService = dcOperationCode.getServiceTypes().stream().anyMatch(serviceType -> serviceType.getName().equals(ServiceTypeEnum.SE.toString()));
                            		if(containsBpService && this.dealerNumber < 32000) {
                            			issueListD3.add("E4: THE OPERATION CODE DOES NOT APPLY TO THE RANGE OF DEALER SENT");
                            		}
                            		if(containsSeService && this.dealerNumber > 29999) {
                            			issueListD3.add("E4: THE OPERATION CODE DOES NOT APPLY TO THE RANGE OF DEALER SENT");
                            		}
                            	}
                            }catch(Exception ex){
