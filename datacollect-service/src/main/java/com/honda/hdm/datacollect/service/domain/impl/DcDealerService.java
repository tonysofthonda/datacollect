...
public class DcDealerService extends RecordStatusableService<DcDealer, Long> implements IDcDealerService {
    ...
    public List<DcContactXDealer> getEnabledContacts(DcDealer dealer) {
        List<DcContactXDealer> originalList = dealer.getDcContactList();
        List<DcContactXDealer> contactList = new ArrayList<>();
        if (originalList == null || originalList.isEmpty()) {
            return contactList;
        }
        originalList.stream().filter((contactDealer) -> (contactDealer.getContact().getDcRecordStatusId() == DcRecordStatusEnum.ENABLED)).forEachOrdered((contact) -> {
    ...
    public Page<DcContactXDealer> getEnabledContacts(String dealerNumber, Pageable pageable) {
        List<DcContactXDealer> contacts = getEnabledContacts(dealerNumber);
