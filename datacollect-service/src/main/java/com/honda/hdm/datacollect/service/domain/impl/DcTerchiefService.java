...
public class DcTerchiefService extends RecordStatusableService<DcTerchief, Long> implements IDcTerchiefService {
    ...
    public DcTerchief update(DcTerchief terchief, Long id) {
        DcTerchief currentTerchief = repository.findOne(id);
        if (currentTerchief == null) {
            return null;
        }
        currentTerchief.setFirstName(terchief.getFirstName());
        currentTerchief.setLastName(terchief.getLastName());
        currentTerchief.setMotherLast(terchief.getMotherLast());
        currentTerchief.setNotes(terchief.getNotes());
        return repository.save(currentTerchief);
    }
    ...
    private List<DcContact> getContacts(Long terchiefId) {
        DcTerchief terchief = repository.findOne(terchiefId);
        if (terchief == null) {
            throw new RecordNotFounException("Territory Chief with ID(" + terchiefId + ") not found");
        }
        List<DcContact> originalList = terchief.getDcContactList();
